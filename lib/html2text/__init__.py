...
class HTML2Text(html.parser.HTMLParser):
    ...
    def handle_tag(
        ...
        if tag == 'en-media':
          self.media_hash = attrs['hash'] if 'hash' in attrs else ''
          if self.media_hash != '':
            self.o('![image]({})'.format(os.path.join('xxx', self.media_hash)))
          print(tag, attrs)
    ...
    def o(
        ...
        if not self.quiet:
            ...
            self.out(data)
            self.outcount += 1
        #print('----------------end in o--------------------:', self.outtextlist)
    ...
    def handle_data(self, data: str, entity_char: bool = False) -> None:
        ...
        #---------------add by haoz------------#
        if self.current_tag == 'en-media':
          self.o('![image]({})'.join(os.path.join('xxx', self.media_hash)))
        #---------------add by haoz------------#
